...
5. Model: leave `GEMINI_MODEL` unset. It defaults to `gemini-flash-latest` —
   a rolling alias that stays current automatically — so there's no need to
   pin a specific dated model string unless you have a reason to freeze one.
   Keep `GEMINI_TTS_MODEL` as you have it.
...
3. Set on the service: `GOOGLE_CLOUD_PROJECT=YOUR_PROJECT` (matches
   config.ts's `gcpProject` — `GCP_PROJECT` is not read anywhere in this
   codebase) and `VERTEX_LOCATION` if not the default. No API key — Cloud
   Run's identity is the credential.
...
```
GEMINI_API_KEY=<paid key>        # voice (TTS + transcribe) + fast turns — store via Secret Manager, not a plain env var
GEMINI_PAID=1
GOOGLE_CLOUD_PROJECT=<project>   # Vertex for heavy synthesis + fallback
NODE_ENV=production              # keeps the free-tier block + dev-auth gate armed
```

Note: TTS (`llm/tts.ts`) always calls the AI Studio REST endpoint directly
with an API key — Vertex (Path B) does not cover voice synthesis, so Path A
is required for voice regardless of whether Vertex is also wired up.
