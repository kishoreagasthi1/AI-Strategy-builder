...
export async function voiceRoutes(
  ...
  app.post("/api/voice/tts", async (req, reply) => {
    ...
    try {
      await gateway.checkLimit(ctx.tenantId);
    } catch (err) {
      if (err instanceof GatewayError) {
        ...
        if (err.detail) req.log.error({ detail: err.detail }, "gateway error detail");
  ...
  app.post("/api/voice/transcribe", async (req, reply) => {
    ...
    try {
      const result = await gateway.generate(
        { tenantId: ctx.tenantId, userId: ctx.userId, module: parsed.data.module, clientName: parsed.data.clientName },
        {
          task: "transcribe",
          maxTokens: 8192,
