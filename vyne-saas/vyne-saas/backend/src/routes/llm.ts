...
const CONSULTANT_ONLY_TASKS = new Set(["synthesis", "strategy_deck"]);
...
const GenerateBody = z.object({
  ...
  // automatically by vyneLLM in vyne-client.js (interviewee -> their
  // interview's client; consultant/owner -> the session's active client);
  // call sites never set this directly. Best-effort like `module`/`task` —
  // trusted the same way, not a security boundary (tenant/user identity
  // always comes from the verified token, never the body).
  ...
  app.post("/api/llm/generate", async (req, reply) => {
    ...
    try {
      const result = await gateway.generate(
        { tenantId: ctx.tenantId, userId: ctx.userId, module, clientName },
      ...
      if (err instanceof GatewayError) {
        ...
        if (err.detail) req.log.error({ detail: err.detail }, "llm generate: provider error detail");
