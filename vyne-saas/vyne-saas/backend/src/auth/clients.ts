...
/**
 * V225-audit MEDIUM note (documented, not changed): truncating to 30 chars
 * means two distinct client names that share the same first-30-alphanumeric
 * prefix collide onto the same norm — e.g. two clients whose names differ
 * only after character 30 would be treated as one client for assignment
 * and workspace-key scoping purposes. This was flagged bundled with the
 * synthetic.ts dedupe below. It is NOT changed here: this exact function
 * (30-char truncation) is duplicated independently in every frontend HTML
 * file that writes vynora_briefing_ or vynora_engagement_ keys (grep
 * "substring(0,30)" across frontend HTML files) and is the key-derivation
 * scheme for every already-provisioned tenant's stored module_state rows.
 * Changing the truncation length or algorithm here without a coordinated
 * frontend change AND a data migration would silently split or merge
 * existing clients' data. Tracked as a follow-up requiring a deliberate
 * migration, not a same-session fix. In practice, collisions require two
 * real client names to share a 30-character alphanumeric prefix, which is
 * an uncommon but non-zero risk for firms with many similarly-named
 * clients (e.g. multiple subsidiaries of the same conglomerate).
 */
 ...
 */
const GLOBAL_KEYS = new Set(["vynora_api_key", "vynora_deck_mode"]);
