...
export const PRICE_TABLE: Record<string, { in: number; out: number }> = {
  ...
  // v5.32.1: gemini-vertex/anthropic-vertex moved to these newer model ids
  // (the 404 fix — see those adapters' doc comments). UNCONFIRMED pricing:
  // carried forward from the prior-generation entries above as a same-tier
  // placeholder so a real billed call never silently meters as $0.00 (the
  // exact failure mode the V225 audit fix above this table already
  // documented once) — NOT verified against Vertex/Anthropic's current
  // published pricing. Replace both with real numbers from
  // https://cloud.google.com/vertex-ai/generative-ai/pricing before
  // treating usage_events cost figures for these two models as accurate.
