...
CLAUDE_REGION="${CLAUDE_REGION:-us-east5}"     # Claude's Vertex region
...
deploy_api() {
  ( cd "$(dirname "$0")/../backend" &&
    gcloud run deploy "$SERVICE" \
      ...
      --set-secrets "DATABASE_URL=vyne-database-url:latest" \
      --set-env-vars "NODE_ENV=production,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},VERTEX_LOCATION=${CLAUDE_REGION}" \
...
deploy_frontend() {
  ( cd "$(dirname "$0")/../frontend" && firebase deploy --only hosting )
}
