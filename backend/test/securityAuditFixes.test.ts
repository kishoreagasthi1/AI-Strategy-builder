...
describe("H-3 / M-1 / M-2 — the XSS cluster (v5.32.29)", () => {
  ...
  it("the interviewee-controlled role is escaped at the conflict sinks", () => {
    ...
    // NB: the old `${esc(high.role)}` spread-header interpolation was replaced in
    // v5.33.7 by `${esc(conflictWho(high,low))}` (asserted below), so there is no
    // longer a bare esc(high.role) to check for here — the escape moved, it did
    // not disappear. The two .not.toContain guards below still pin the OLD
    // unescaped forms so they can never come back.
