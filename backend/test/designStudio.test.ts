...
describe("roadmap.html — publishes use-case metadata for cross-module reads (v5.32.18)", () => {
  ...
  it("a failed metadata build preserves the previous map instead of publishing an empty one", () => {
    ...
    // v5.33.12: the gate is unchanged (only overwrite when the catalog is ready
    // OR nothing is selected), but the map is now MERGED onto the previous one
    // via mergeUcMeta rather than replaced, so a custom (ai_uc_*) use case the
    // rebuilt catalog no longer contains keeps its published name instead of
    // degrading to a raw id. Still preserves the previous map on a failed build.
    expect(src).toContain("if(Object.keys(fresh).length || !Object.keys(selected||{}).length) ucMeta = mergeUcMeta(prevEntry.ucMeta || {}, fresh);");
  });
});
...
describe("solution_design.html — reads the roadmap selection directly (v5.32.18)", () => {
  ...
  it("reads vynora_roadmap_state rather than importing an uploaded snapshot file", () => {
    ...
    // v5.33.11: roadmap.html moved each client's roadmap into a PER-ENGAGEMENT
    // key (vynora_roadmap_state_<engKey>), so this module reads that first and
    // uses its `entry` ...
    expect(src).toContain("vyneStore.getItem('vynora_roadmap_state_'+out.engKey)");
    expect(src).toContain("if(part && part.entry) mine=part.entry;");
    // ... and falls back to the single legacy blob's byEng slice only for
    // clients not yet migrated to the split.
    expect(src).toContain("st=JSON.parse(vyneStore.getItem('vynora_roadmap_state')||'{}')");
    expect(src).toContain("mine=(st.byEng||{})[out.engKey]||{};");
