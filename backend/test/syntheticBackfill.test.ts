...
describe.skipIf(!ENABLED)("synthetic login backfill", () => {
  ...
  it("uses the uid a later regeneration will also derive", async () => {
    ...
    // v5.33.4 (audit 5332-9): the uid is TENANT-KEYED — synthetic:<tenant>:<code>:<person>
    // — because `users` is global and engagement codes are unique only per tenant, so
    // the old synthetic:<code>:<person> form collided across firms. The tenant id is
    // random per run, so derive the segment the same way syntheticLogin() does rather
    // than hardcode it; the determinism the test guards is unchanged.
    const tslug = tenant.toLowerCase().replace(/[^a-z0-9]/g, "");
    expect(uid.rows[0].identity_platform_uid).toBe(`synthetic:${tslug}:alphsyn1:victoria.hale.synthetic`);
