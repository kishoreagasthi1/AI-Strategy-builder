...
async function commitSyntheticEngagement(args: {
  ...
  await withTenant(ctx.tenantId, async (c) => {
     ...
     *
     * v5.33.10 — synthetic=true, and why it is not optional. This row has a
     * FIXED state_module (iv_synth_<code>_followup). Created without the
     * synthetic flag it defaulted to FALSE, so on the NEXT generation for this
     * client the synthetic-only DELETE above skipped it, the re-insert here hit
     * a duplicate on uq_interviews_tenant_state_module, and commit threw — the
     * whole thing surfaced to the consultant as "Retry failed: internal_error"
     * the instant they regenerated or retried a partial run. It is practice
     * data like every other synthetic row (migration 026 already backfills
     * iv_synth_* rows to synthetic=true); marking it so lets regeneration
     * replace it cleanly instead of colliding with its own previous self. */
...
export async function syntheticRoutes(app: FastifyInstance, gateway: LlmGateway): Promise<void> {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          ...
          const result = await gateway.generate(
            { tenantId: ctx.tenantId, userId: ctx.userId, module: "synthetic_data" },
            {
              ...
              // v5.33.10: 4000 was enough for an initial interview but the
              // REFRESH response is the largest the generator produces — a full
              // 12–16 turn transcript PLUS findings that recap what changed —
              // and the longest personas truncated mid-array at 4000, failed to
              // parse both attempts, and surfaced as the opaque internal_error.
              // claude-sonnet-5 allows well above this; give it real headroom.
              maxTokens: 8000,
  ...
  async function loadClientContext(
    ...
    const norm = normClient(clientName);
    const ws = await withTenant(ctx.tenantId, async (c) => {
      /* v5.33.10. The briefing may live under the CODE-shaped key
       * (vynora_briefing_<code>) rather than the norm-shaped one: the
       * Pre-Engagement page writes code-first via vyneWriteClientKey once the
       * client has an engagement code, and vyneReadClientKey accepts either.
       * Reading norm-ONLY here is exactly why a client whose briefing had
       * migrated to the code key generated against the GENERIC 5 personas
       * instead of their own roleCatalog. Resolve the code from the index
       * first, then fetch the briefing under BOTH shapes and prefer the code
       * one — the same precedence the frontend's vyneReadClientKey uses. */
      const idxR = await c.query<{ value: { v: string } }>(
        `SELECT value FROM module_state WHERE module = 'workspace' AND key = 'vynora_engagement_index'`
      );
      let idx: Record<string, string> = {};
      try { idx = JSON.parse(idxR.rows[0]?.value.v ?? "{}"); } catch { /* fresh */ }
      const codeForClient = idx[norm];
      const briefingKeys = ["vynora_briefing_" + norm];
      if (codeForClient) briefingKeys.push("vynora_briefing_" + codeForClient);
      const r = await c.query<{ key: string; value: { v: string } }>(
        `SELECT key, value FROM module_state WHERE module = 'workspace' AND key = ANY($1)`,
        [briefingKeys]
      );
      const out: Record<string, string> = { vynora_engagement_index: JSON.stringify(idx) };
      for (const row of r.rows) out[row.key] = row.value.v;
      return out;
    });
    let idx0: Record<string, string> = {};
    try { idx0 = JSON.parse(ws["vynora_engagement_index"] ?? "{}"); } catch { /* none */ }
    const codeForClient = idx0[norm];
    let briefing0: { industry?: string; roleCatalog?: BriefingRole[]; hypotheses?: { text?: string }[] } | null = null;
    // Code-first, then norm — the same precedence as the frontend's vyneReadClientKey.
    try {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      try {
        out = await commitSyntheticEngagement({
          ctx: { tenantId: ctx.tenantId, userId: ctx.userId },
          clientName, norm, industry, code, existingCode,
          hasRealBriefing: Boolean(briefing0),
        ...
        /* v5.33.10. commitSyntheticEngagement runs buildSynthesis, a
         * transaction-scoped advisory lock, and a delete-then-insert across
         * interviews + interview_transcripts. Any throw here previously reached
         * server.ts's catch-all and hit the browser as the opaque
         * "internal_error" — which is exactly what a consultant saw as "Retry
         * failed: internal_error", with no way to tell a generation failure
         * from a persistence one. Log the real error in full (never sent), and
         * return a stable, specific code plus how many interviews were in hand
         * so the client can say what it managed to generate. */
        req.log.error({ err, clientName, interviews: interviews.length }, "synthetic commit failed");
        reply.code(502).send({ error: "commit_failed", generated: interviews.length });
        return;
      }
