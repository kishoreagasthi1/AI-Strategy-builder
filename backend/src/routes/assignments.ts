...
import {
  invalidateNorms, invalidateTenantNorms, invalidateMembershipUid,
} from "../auth/cache.js";
...
const TeamBody = z.object({
  email: z.string()
    .min(3, "enter a valid email (at least 3 characters)")
    .max(200, "must be 200 characters or fewer"),
  name: z.string().max(200, "must be 200 characters or fewer").optional(),
...
export async function assignmentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      if (!parsed.success) {
        // Was a bare "invalid_input" — the owner had no way to tell which
        // field failed (usually a password under the 10-char minimum).
        const detail = parsed.error.issues
          .map((i) => {
            const field = i.path.join(".") || "input";
            const label = field === "email" ? "Email"
                        : field === "name" ? "Name"
          ...
          .join("; ");
        reply.code(400).send({ error: "invalid_input", detail });
        return;
      }
      ...
      // A membership was inserted for this uid; drop any cached resolution so a
      // (possibly cross-tenant) stale entry can't outlive the change (v5.34.0).
      invalidateMembershipUid(idpUid);
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      // Revoke cached access immediately on THIS instance (v5.34.0): the
      // consultant's membership and scope are gone, so drop both caches rather
      // than let a hit keep them alive until the TTL. info.uid is the IdP uid
      // the membership cache is keyed on.
      invalidateNorms(ctx.tenantId, member.id);
      if (info?.uid) invalidateMembershipUid(info.uid);
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      // consultants; a targeted per-user invalidation can't enumerate them, so
      // clear the whole tenant's scope cache (v5.34.0).
      invalidateTenantNorms(ctx.tenantId);
  ...
  app.patch(
    ...
    async (req, reply) => {
      ...
      // Assignments moved onto the new norm, so any consultant's cached
      // allowed-norm set for this firm is now stale (v5.34.0).
      invalidateTenantNorms(ctx.tenantId);
  ...
  app.post(
    ...
    async (req, reply) => {
      ...
      invalidateNorms(ctx.tenantId, member.id); // scope grew (v5.34.0)
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      invalidateNorms(ctx.tenantId, member.id); // scope shrank (v5.34.0)
