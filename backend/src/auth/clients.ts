...
export async function allowedClientNorms(
  ...
  /*
   * v5.34.0 (perf review P1): this same assignment set was re-fetched on every
   * module-state/billing/scorecard request. It is cached per-instance on
   * (tenant, user) for a short TTL and invalidated at the assignment/team
   * mutation sites (assignments.ts). cachedAllowedNorms hands back a fresh Set
   * on every hit, so the returned scope is never shared across requests. Both
   * imports stay LAZY so this file remains importable by the plain-node rename
   * tests (see the module header) — neither pg nor the cache is pulled at load.
   */
  const { cachedAllowedNorms } = (await import("./cache.js")) as typeof import("./cache.js");
  return cachedAllowedNorms(tenantId, userId, async () => {
...
const CODE_SUFFIX = [
  ...
  /*
   * v5.33.13 — persisted custom (ai_uc_*) use cases, one key per engagement,
   * suffixed by roadmap.html's getEngKey() ('eng_<CODE>' / 'client_<norm>').
   * Client-owned like every roadmap family; without this line resolveKeyClient()
   * returns UNKNOWN for it and a scoped consultant's custom use cases would be
   * silently dropped on read — the same class of loss the families above fixed.
   */
  "vynora_custom_ucs_",
  /*
   * v5.33.20 — cached semantic clustering of shared capability prerequisites,
   * one key per engagement (getEngKey()). Client-owned like every roadmap family;
   * without this a scoped consultant's cached grouping is dropped on read and the
   * Shared Prerequisites panel silently re-runs the LLM pass every open.
   */
  "vynora_shared_prereq_clusters_",
];
