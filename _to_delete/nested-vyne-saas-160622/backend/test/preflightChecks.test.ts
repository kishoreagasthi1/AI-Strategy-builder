...
function installBsdDateShim(): string {
  ...
  writeFileSync(shim, [
    ...
    "    *) exit 1 ;;",
    "  esac",
    "  # Turn the bare stamp into an epoch, honouring the CURRENT timezone (the",
    "  # -4h bug). Portable across BOTH /bin/date flavours so this shim — and so",
    "  # `npx vitest run` in the deploy gate — works on a macOS/BSD deploy box,",
    "  # not just Linux/CI: GNU accepts -d; BSD (real, on macOS) accepts -j -f.",
    '  /bin/date -d "$stamp" "$out" 2>/dev/null && exit 0',
    '  exec /bin/date -j -f "%Y-%m-%dT%H:%M:%S" "$stamp" "$out"',
    "fi",
    'exec /bin/date "$@"',
  ].join("\n"));
