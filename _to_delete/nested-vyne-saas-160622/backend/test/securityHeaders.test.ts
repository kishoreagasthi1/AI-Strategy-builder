...
describe("CSP covers every origin the app actually connects to (v5.32.37)", () => {
  const fbJson = readFileSync(new URL("../../frontend/firebase.json", import.meta.url), "utf8");

  it("allows the Gemini Live WebSocket in the production CSP", () => {
    // Realtime voice opens wss://generativelanguage.googleapis.com directly
    // from the browser. Adding that connection without adding it here is a
    // silent, total failure: the token mints, the code runs, and Chrome refuses
    // the socket. It cost a full deploy cycle to find.
    expect(fbJson).toContain("wss://generativelanguage.googleapis.com");
  });

  it("allows data: audio, which the TTS path plays through an <audio> element", () => {
    // With no media-src, media falls back to default-src 'self' — which does
    // NOT include data:, so synthesized speech was blocked by policy.
    expect(fbJson).toMatch(/media-src[^;]*data:/);
  });

  it("still refuses an arbitrary third-party origin", () => {
    // The point of connect-src is blocking exfiltration. Widening it for the
    // Live API must not have turned into a wildcard.
    expect(fbJson).not.toContain("connect-src *");
    expect(fbJson).not.toMatch(/connect-src[^;]*\shttps:(?:\s|;)/);
  });
});
