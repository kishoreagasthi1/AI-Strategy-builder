...
describe.skipIf(!ENABLED)("Row-Level Security tenant isolation", () => {
  ...
  beforeAll(async () => {
    ...
    await app.connect();
    // engagements.code is NOT NULL since migration 025 — every insert supplies one.
    for (const [tid, name, code] of [
      [tenantA, "Client of A", "RLSA"],
      [tenantB, "Client of B", "RLSB"],
    ] as const) {
      await app.query("BEGIN");
      await app.query("SELECT set_config('app.tenant_id', $1, true)", [tid]);
      const r = await app.query(
        `INSERT INTO engagements (tenant_id, code, client_name)
         VALUES (current_setting('app.tenant_id', true)::uuid, $1, $2) RETURNING id`,
        [code, name]
      );
  ...
  it("Tenant A cannot INSERT a row claiming Tenant B's tenant_id (WITH CHECK)", async () => {
    ...
    await expect(
      // code supplied so the row reaches the RLS WITH CHECK (a missing NOT NULL
      // column would reject first, with a different error than the one asserted).
      app.query(`INSERT INTO engagements (tenant_id, code, client_name) VALUES ($1, 'SMUG', 'smuggled')`, [tenantB])
