...
import { cachedMembership } from "./cache.js";
...
export function makeAuthHook(verifier: TokenVerifier) {
  return async function authHook(req: FastifyRequest, reply: FastifyReply): Promise<void> {
     ...
     */
    /*
     * v5.34.0 (perf review P1): the membership resolution below ran on EVERY
     * protected request. It is cached per-instance on (verified uid, idp
     * tenant) for a short TTL, and invalidated at the team/assignment mutation
     * sites (assignments.ts). Only a real row is cached — a "no membership"
     * result always re-queries, so a just-added user is never negatively
     * cached. See auth/cache.ts for the cross-instance staleness reasoning.
     */
    const row = await cachedMembership(
      identity.uid,
      identity.idpTenantId ?? null,
      () =>
