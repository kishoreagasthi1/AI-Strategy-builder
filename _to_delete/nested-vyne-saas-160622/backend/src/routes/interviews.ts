...
import { invalidateMembershipUid } from "../auth/cache.js";
  ...
  app.delete(
    ...
    async (req, reply) => {
      ...
      if (deleted) {
        ...
        // v5.34.3: deleting the interviewee's last interview revoked their
        // membership, so drop the cached membership immediately — otherwise the
        // short-TTL auth cache (v5.34.0) would let this revoked login pass
        // authHook for up to TTL_MS and reach /mine/bootstrap. removed.uid is
        // the IdP uid the membership cache is keyed on. (Caught by the harness:
        // interviews.test.ts "consultant deletes an interview".)
        if (removed?.uid) invalidateMembershipUid(removed.uid);
