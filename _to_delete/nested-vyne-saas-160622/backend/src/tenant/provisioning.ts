...
export async function provisionFirm(input: ProvisionInput): Promise<ProvisionResult> {
  ...
  try {
    ...
    const result = await withoutTenant(async (c) => {
      ...
      try {
        ...
        const t = await c.query<{ id: string }>(
          // v5.32.29 (audit CR-3): monthly_token_limit was never written by
          // any code path, so every firm was provisioned with NULL and
          // dbLimitCheck's "NULL = unlimited" branch made the whole spend cap
          // inert. It now comes from the plan the tenant starts on.
          `INSERT INTO tenants (name, idp_tenant_id, slug, monthly_token_limit)
           VALUES ($1, $2, $3,
                   COALESCE((SELECT monthly_token_limit FROM subscription_plans WHERE key = 'trial'),
                            3000000))
           RETURNING id`,
    ...
    // Audit write happens AFTER commit, in its own transaction — audit_log
    // is now RLS-protected (migration 008), which needs app.tenant_id set,
    // and at this exact point (mid the withoutTenant transaction above) the
    // tenant row exists but no tenant context has been established for RLS
    // purposes. Best-effort: a logging failure must never undo a successful
    // provision (see audit/log.ts's doc comment).
    await auditLog(result.tenantId, result.ownerUserId, "tenant_provisioned", { firmName: input.firmName });
    return result;
  } catch (err) {
