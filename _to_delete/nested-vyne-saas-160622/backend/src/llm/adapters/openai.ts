...
import { estimateCost, normalizeFinishReason } from "../types.js";
  ...
  return {
    ...
    async generate(req: GenerateRequest) {
      ...
      return {
        ...
        model,
        /*
         * v5.32.65 (audit V2-L7). Dropped on the floor here, exactly as the
         * Gemini adapters dropped it before v5.32.23 — and for the same cost:
         * nothing downstream could tell a complete answer from one cut off at
         * max_tokens, so a truncated interview summary or half a solution
         * design read as finished work. normalizeFinishReason maps OpenAI's
         * "length" onto the same vocabulary the other adapters emit, so callers
         * do not have to know which provider answered.
         */
