...
describe.skipIf(!ENABLED)("adversarial RLS penetration (as the production app role)", () => {
  ...
  beforeAll(async () => {
    ...
    for (const t of [A, B]) {
      await admin.query(`INSERT INTO engagements (tenant_id, client_name, industry) VALUES ($1,'Pen Client','tech')`, [t]);
      await admin.query(
  ...
  it("tenant A cannot forge tenant B's id on INSERT, nor move a row across the boundary", async () => {
    const attacks: [string, string][] = [
      ["INSERT forging B", `INSERT INTO engagements (tenant_id, client_name) VALUES ('${B}','forged')`],
