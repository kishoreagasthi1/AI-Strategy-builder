...
describe.skipIf(!ENABLED)("Synthetic engagement generator", () => {
  ...
  it("generating a second time for the SAME client replaces rather than 500s", async () => {
    ...
    const rows = await admin.query<{ n: number }>(
      `SELECT count(*)::int AS n FROM interviews WHERE client_name = 'Followup Testco'`);
    // Five personas plus one follow-up: the previous ten-interview two-round
    // set is gone, not stacked underneath.
