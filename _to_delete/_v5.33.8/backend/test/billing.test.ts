...
describe.skipIf(!ENABLED)("Billing HTTP — client scoping", () => {
  ...
  // Regression (reported live: "View statement" → "Failed: internal_error"):
  // node-postgres parses `timestamptz` into a JS Date object, not a string,
  // even though fetchUsageEvents's row type and UsageEventLite.createdAt
  // both declare it as `string`. buildBillingStatement's
  // `.sort((a,b) => a.createdAt.localeCompare(b.createdAt))` calls a
  // String-only method that doesn't exist on Date — but Array.sort() never
  // invokes its comparator for arrays of 0 or 1 items, so every other test
  // above (each client seeded with exactly one row) passed anyway. Any real
  // client with 2+ billable calls hit the TypeError, which escaped
  // uncaught (past this route's own try/catch, which only wraps the DB
  // call) straight to the generic 500 "internal_error" handler. This client
  // is seeded last, after the tests above that assert the exact seeded
  // client set, so it doesn't disturb their expectations.
  it("statement: a client with 2+ billable calls sorts and totals without crashing", async () => {
    await admin.query("BEGIN");
    await admin.query("SELECT set_config('app.tenant_id', $1, true)", [tenant]);
    await admin.query(
      `INSERT INTO usage_events (tenant_id, module, task, provider, model, tokens_in, tokens_out, cost_est_usd, ok, client_name, client_norm, created_at)
       VALUES ($1,'m','first-call','p','mo',100,50,0.01,true,'Gamma','gamma','2026-01-01T00:00:00Z'),
              ($1,'m','second-call','p','mo',200,100,0.02,true,'Gamma','gamma','2026-01-02T00:00:00Z')`,
      [tenant]
    );
    await admin.query("COMMIT");

    const r = await app.inject({
      method: "GET", url: "/api/billing/statement?client=Gamma",
      headers: { authorization: "Bearer tok-owner" },
    });
    expect(r.statusCode).toBe(200);
    const body = r.json();
    expect(body.callCount).toBe(2);
    expect(body.lineItems.map((li: any) => li.task)).toEqual(["first-call", "second-call"]);
    expect(body.totalCostUsd).toBeCloseTo(0.03);
  });
});

describe("live-session holds never reach a client invoice (v5.32.44)", () => {
  it("excludes hold and release rows from the billed events", async () => {
    const { buildBillingSummary } = await import("../src/routes/billing.js");
    const { TASK_HOLD, TASK_HOLD_RELEASE, TASK_ACTUAL, NON_BILLABLE_TASKS } =
      await import("../src/llm/liveSession.js");

    // The route filters these out in SQL; this asserts the contract that filter
    // depends on. A hold is a pre-authorisation for what a session COULD cost —
    // 45 minutes of audio in both directions, about $1.42. Billing sums
    // usage_events, so leaving holds in charged a client $1.51 for a
    // twenty-second conversation.
    expect(NON_BILLABLE_TASKS).toEqual(
      expect.arrayContaining([TASK_HOLD, TASK_HOLD_RELEASE]));
    expect(NON_BILLABLE_TASKS).not.toContain(TASK_ACTUAL);

    // And the summary itself still totals whatever it is handed.
    const rows = [
      { clientName: "Acme", clientNorm: "acme", module: "m", task: TASK_ACTUAL,
        provider: "gemini-live", model: "x", tokensIn: 100, tokensOut: 200,
        costEstUsd: 0.05, ok: true, createdAt: new Date().toISOString() },
    ] as any;
    const out = buildBillingSummary(rows);
    expect(out[0].tokensIn + out[0].tokensOut).toBe(300);
  });
});

describe("legacy hold rows are excluded too (v5.32.46)", () => {
  it("treats the pre-v5.32.44 task names as non-billable", async () => {
    const { NON_BILLABLE_TASKS, LEGACY_HOLD_TASKS } = await import("../src/llm/liveSession.js");
    // Rows written by the first version of the hold mechanism already exist on
    // live deployments. Without this, a client's statement shows a $1.42
    // pre-authorisation forever, for a conversation that cost about a cent.
    for (const t of LEGACY_HOLD_TASKS) expect(NON_BILLABLE_TASKS).toContain(t);
    expect(NON_BILLABLE_TASKS).toContain("live_session_reserve");
    expect(NON_BILLABLE_TASKS).toContain("live_session_refund");
  });
});
