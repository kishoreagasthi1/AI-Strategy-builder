...
(function () {
  ...
  function hydrateSync() {
    ...
    if (INTERVIEWEE) { hydrateIntervieweeSync(); HYDRATED = true; return; }
    // v5.32.25 DATA LOSS. fetchStateSync returns null on ANY failure — 500,
    // 502, timeout, dropped wifi — and this used to collapse that to {}. The
    // page then booted looking exactly like a brand-new firm, and the first
    // save did a read-modify-write of the whole blob from that empty cache:
    // roadmap.html's savePersistentState would write a vynora_roadmap_state
    // containing only the current engagement, wiping every other client's
    // notes, Gantt, synthesis and selections. One Cloud Run cold start during
    // page load was enough, and the save pill said "Saved".
    //
    // Now a failed hydration is distinguishable from an empty firm, and
    // vyneStore refuses to write until a later hydration succeeds.
  ...
  window.vyneStore = {
    ...
    isHydrated: function () { return HYDRATED || INTERVIEWEE; },
